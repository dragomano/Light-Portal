...
it('should throw exception for unknown entity type', function () {
    expect(fn() => $this->factory->createMockData('unknown'))
        ->toThrow(
            ...
            'Unknown entity type: unknown. Available types: blocks, pages, categories, tags, params, translation'
        );
});
...
it('should create mock DatabaseInterface', function () {
    $database = $this->factory->createMockDatabase();

    expect($database)->toBeInstanceOf(DatabaseInterface::class);
});
...
it('should create test environment without database', function () {
    ...
    expect($environment)->toHaveKey('data')
        ->and($environment)->toHaveKey('request')
        ->and($environment)->not->toHaveKey('database');
});
...
it('should cleanup environment', function () {
    ...
    expect($newEnvironment)->toHaveKey('data')
        ...
        ->and($newEnvironment)->toHaveKey('database');
});
