...
class MainMenu extends Plugin
{
	...
	public function currentAction(string &$current_action)
	{
		...
		if (Helper::request()->url() === $context['canonical_url'] && in_array($context['canonical_url'], array_column($context['lp_main_menu_addon_items'], 'url'))) {
			$current_action = 'portal_action_' . $current_action;

			if (Helper::request()->isEmpty('action') && Helper::request()->notEmpty(LP_PAGE_PARAM)) {
				$current_action = 'portal_page_' . Helper::request(LP_PAGE_PARAM);
			}
		}
	}
	...
	public function saveSettings(array &$plugin_options)
	{
		...
		if (Helper::post()->has('url')) {
			foreach (Helper::post('url') as $key => $value) {
				foreach (Helper::post('langs') as $lang => $val) {
					if (! empty($val[$key]))
						$langs[$key][$lang] = $val[$key];
				}

				$items[] = array(
					'url'     => Helper::validate($value, 'url'),
					'unicode' => Helper::validate(Helper::post('unicode')[$key]),
					'langs'   => $langs[$key],
					'access'  => Helper::validate(Helper::post('access')[$key], 'int')
				);
			}
		}
