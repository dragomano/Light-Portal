...
class ChosenTopicArticle implements ArticleInterface
{
	/**
	...
	public function getData(int $start, int $limit)
	{
		...
		$selected_topics = !empty($modSettings['lp_frontpage_topics']) ? explode(',', $modSettings['lp_frontpage_topics']) : [];
		...
		if (($topics = Helpers::cache()->get('articles_u' . $user_info['id'] . '_' . $start . '_' . $limit, LP_CACHE_TIME)) === null) {
			...
			$request = $smcFunc['db_query']('', '
				...
				WHERE t.approved = {int:is_approved}
					...
					AND t.id_topic IN ({array_int:selected_topics})
