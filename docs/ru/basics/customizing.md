...
* Шаблон оформления статей — можно выбрать предпочитаемый шаблон для отображения карточек. Для добавления своих шаблонов поместите функцию `template_show_articles_любоеназвание` (замените `любоеназвание` каким-нибудь красивым словом на латинице) в файл `/Themes/default/LightPortal/ViewFrontPage.template.php`.
