...
final class Block
{
	public function show()
	{
		...
		if (! empty($modSettings['lp_hide_blocks_in_admin_section']) && Helper::request()->is('admin'))
			return;
		...
		foreach ($blocks as $item => $data) {
			if (Helper::canViewItem($data['permissions']) === false)
				continue;

			$data['can_edit'] = $context['user']['is_admin'] || ($context['allow_light_portal_manage_own_blocks'] && $data['user_id'] == $context['user']['id']);

			empty($data['content'])
				? Helper::prepareContent($data['content'], $data['type'], $data['id'], LP_CACHE_TIME)
				: Helper::parseContent($data['content'], $data['type']);
			...
			$title = Helper::getTranslatedTitle($data['title']);
			$icon  = Helper::getIcon($context['lp_blocks'][$data['placement']][$item]['icon']);
	...
	public static function getActive(): array
	{
		...
		if (($active_blocks = Helper::cache()->get('active_blocks')) === null) {
			...
			Helper::cache()->put('active_blocks', $active_blocks);
		}
	...
	private function getFilteredByAreas(): array
	{
		...
		$area = $context['current_action'] ?: (empty($modSettings['lp_frontpage_mode']) ? 'forum' : LP_ACTION);

		if (! empty($modSettings['lp_standalone_mode']) && ! empty($modSettings['lp_standalone_url'])) {
			if ($modSettings['lp_standalone_url'] === Helper::request()->url()) {
				$area = LP_ACTION;
		...
		if (! empty($context['lp_page']) && Helper::isFrontPage($context['lp_page']['alias']))
			$area = LP_ACTION;
