...
describe('ParamsTableCreator', function () {
    beforeEach(function () {
        ...
        $this->creator = new ParamsTableCreator($this->adapter);
    ...
    it('returns correct table name', function () {
        $reflection = new ReflectionProperty(ParamsTableCreator::class, 'tableName');
    ...
    it('returns correct full table name', function () {
        $reflection = new ReflectionMethod(ParamsTableCreator::class, 'getFullTableName');

        $result = $reflection->invoke($this->creator);

        expect($result)->toBe('smf_lp_params');
    });
        ...
        $this->sql
            ...
            ->andReturn(
                'CREATE TABLE smf_lp_params (
                    ...
                    value TEXT NOT NULL,
                    UNIQUE KEY item_id_type_name (item_id, type, name)
                )'
            );
    ...
    it('inserts default data', function () {
        ...
        $select->shouldReceive('where')->with(['item_id' => 1, 'type' => 'page', 'name' => 'show_author_and_date'])->andReturnSelf();
        $select->shouldReceive('columns')->with(['count' => new Expression('COUNT(*)')], false)->andReturnSelf();

        $this->sql->shouldReceive('select')->with('lp_params')->andReturn($select);
        ...
        $insert->shouldReceive('columns')->with(['item_id', 'type', 'name', 'value'])->andReturnSelf();
        $insert->shouldReceive('values')->with([1, 'page', 'show_author_and_date', 0])->andReturnSelf();

        $this->sql->shouldReceive('insert')->with('lp_params')->andReturn($insert);
    ...
    it('does not insert default data if exists', function () {
        ...
        $select
            ->shouldReceive('where')
            ->with(['item_id' => 1, 'type' => 'page', 'name' => 'show_author_and_date'])
            ->andReturnSelf();
        $select
