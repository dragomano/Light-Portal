...
class BoardArticle extends AbstractArticle
{
	...
	public function init()
	{
		...
		Addon::run('frontBoards', array(&$this->columns, &$this->tables, &$this->wheres, &$this->params, &$this->orders));
	}

	public function getData(int $start, int $limit): array
	{
		global $smcFunc, $user_info, $modSettings, $context, $scripturl, $txt;
		...
		while ($row = $smcFunc['db_fetch_assoc']($request)) {
			...
			if (! empty($modSettings['lp_show_images_in_articles'])) {
				$image = Helper::getImageFromText($description);

				if (! empty($row['attach_id']) && empty($image)) {
					$image = $scripturl . '?action=dlattach;topic=' . $row['id_topic'] . ';attach=' . $row['attach_id'] . ';image';
				}
			...
			if (! empty($modSettings['lp_show_teaser']))
				$boards[$row['id_board']]['teaser'] = Helper::getTeaser($description);
			...
			Addon::run('frontBoardsOutput', array(&$boards, $row));
		}
