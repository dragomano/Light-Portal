...
class ChosenPageArticle implements ArticleInterface
{
	/**
	...
	public function getData(int $start, int $limit)
	{
		...
		$selected_pages = !empty($modSettings['lp_frontpage_pages']) ? explode(',', $modSettings['lp_frontpage_pages']) : [];
		...
		if (($pages = Helpers::cache()->get('articles_u' . $user_info['id'] . '_' . $start . '_' . $limit, LP_CACHE_TIME)) === null) {
			...
			$request = $smcFunc['db_query']('', '
				...
				WHERE p.status = {int:status}
					...
					AND p.page_id IN ({array_int:selected_pages})' . (!empty($custom_wheres) ? '
					' . implode("\n\t\t\t\t\t", $custom_wheres) : '') . '
		...
		if (($num_pages = Helpers::cache()->get('articles_u' . $user_info['id'] . '_total', LP_CACHE_TIME)) === null) {
			$request = $smcFunc['db_query']('', '
				...
				WHERE status = {int:status}
					...
					AND page_id IN ({array_int:selected_pages})',
				array(
					'status'         => Page::STATUS_ACTIVE,
					'current_time'   => time(),
					'permissions'    => Helpers::getPermissions(),
