						...
						<tbody>
							<tr class="windowbg">
								<td colspan="2" style="cursor: move">
									<button type="button" class="button" @click="removeSet(index)">
								...
								<td x-id="['select']">
									...
									<select x-model="dataset.type" name="set_type[]" :id="$id('select')">
										<template x-for="(value, key) in charts" :key="key">
											<option :value="key" :selected="key === dataset.type" x-text="value"></option>
										</template>
									</select>
								</td>
						...
						<template x-if="!usePalette">
							<tr class="windowbg">
								<td x-id="['input-color']">
									...
									<input type="color" x-model="dataset.backgroundColor" :id="$id('input-color')">
									<input
								...
								<td x-id="['input-color']">
									...
									<input type="color" x-model="dataset.borderColor" :id="$id('input-color')">
									<input
...
<script>
	...
	function handleDataSets() {
		return {
			...
			charts: @json($context['lp_chart_types'] ?? []),
