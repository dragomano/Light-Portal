...
class Optimus extends Plugin
{
	...
	{
		if (empty($this->modSettings['lp_optimus_addon_use_topic_descriptions']) || ! class_exists('\Bugo\Optimus\Integration'))
			return;
	...
	public function frontTopicsOutput(array &$topics, array $row)
	{
		...
		if (! empty($this->modSettings['lp_optimus_addon_show_topic_keywords']))
			$topics[$row['id_topic']]['tags'] = $this->cache('topic_keywords')->setFallback(__CLASS__, 'getKeywords', (int) $row['id_topic']);

		if (! empty($this->modSettings['lp_optimus_addon_use_topic_descriptions']) && ! empty($row['optimus_description']) && ! empty($topics[$row['id_topic']]['teaser']))
