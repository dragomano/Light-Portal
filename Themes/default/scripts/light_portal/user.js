class Page {
	async like(parent, target) {
		ajax_indicator(true);

		let response = await fetch(target.href + ';js=1');
	...
	showLikers(parent, target) {
		return reqOverlayDiv(target.href + ';js=1', parent.textContent, 'post/thumbup.png');
	}
}
...
class Comment {
	...
	focus(target, refs) {
		...
		if (refs.toolbar) {
			refs.toolbar.style.display = 'block';
		}
	...
	async add(target, refs) {
		...
		if (! window.location.search) {
			if (window.location.pathname.match(/start./i) && parseInt(window.location.pathname.split('start.')[1].match(/\d+/) ?? 0) == refs.comment_form.start.value) {
			...
			} else if (hasStartParam && parseInt(window.location.search.split('start=')[1].match(/\d+/) ?? 0) == refs.comment_form.start.value) {
				window.location.hash = '#comment' + data.item;
			} else {
				window.location.hash = '';
